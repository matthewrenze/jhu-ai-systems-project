...
output_folder_path = root_folder_path + "\\Data\\Training\\Intro to Data for Data Science\\Slides\\"
...
for input_file_path in input_file_paths:
    ...
    # Remove the notes with an empty string
    notes_regex = re.compile(r"\n<!--\n([\s\S]+?)\n-->(?:\n|$)")
    notes_matches = notes_regex.findall(output)
    output = notes_regex.sub("", output)

    # Create the output file name
    parent_folder_name = os.path.basename(os.path.dirname(input_file_path))
    input_file_name = os.path.basename(input_file_path)
    output_file_name = parent_folder_name[0] + "-" + input_file_name[0] + " - slides.md"
